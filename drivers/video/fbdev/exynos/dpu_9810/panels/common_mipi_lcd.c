...
int mipi_write(u32 id, u8 cmd_id, const u8 *cmd, u8 offset, int size)
{
	int ret, retry = 3;
	unsigned long d0;
	u32 type, d1;
	...
	while (--retry >= 0) {
		if (offset > 0) {
			if (dsim_write_data(dsim,
						MIPI_DSI_DCS_SHORT_WRITE_PARAM, 0xB0, offset)) {
		...
		if (dsim_write_data(dsim, type, d0, d1)) {
...
int mipi_read(u32 id, u8 addr, u8 offset, u8 *buf, int size)
{
	int ret, retry = 5;
	struct dsim_device *dsim = get_dsim_drvdata(id);
