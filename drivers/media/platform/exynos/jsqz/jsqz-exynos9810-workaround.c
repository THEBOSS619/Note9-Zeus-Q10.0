/* drivers/media/platform/exynos/jsqz/jsqz-exynos9810-workaround.c
 *
 * Implementation of functions needed for working around an Exynos9810 HW bug.
 *
 * This is part of the jsqz HW encoder kernel driver for Exynos SoCs.
 *
 ...
 *
 * Exynos9810 HW jsqz chip will access memory out-of-bounds when img
 * height/width is not multiple of the blk size (although it will not
 * actually touch that memory).
 *
 * The size of the buffer that Exynos9810 jsqz HW encoder will access
 ...
 *	-EINVAL otherwise.
 */
static int jsqz_exynos9810_img_buffer_size(struct device *dev,
...
int jsqz_exynos9810_setup_workaround(struct device *dev,
	 ...
	 */
	struct jsqz_buffer_plane_dma *plane = &task->dma_buf_out->plane;

	/* the size of the buffer that Exynos9810 jsqz encoder expects to be
	...
	*rounded_up_height = roundup_to_blksize(dev, img_info->height, y_blk_size, &okay);
	...
	ret = jsqz_exynos9810_img_buffer_size(dev, task,
					      *rounded_up_width,
					      *rounded_up_height,
