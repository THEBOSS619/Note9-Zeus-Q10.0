...
static u32 get_input_boost_freq(struct cpufreq_policy *policy, u32 state)
{
	u32 freq, cpu;
			   ...
			   unsigned long action, void *data)
{
	...
	if (get_input_boost_freq(policy, state) == 0)
		return NOTIFY_OK;
	...
	if (state & INPUT_BOOST || state & GENERAL_BOOST) {
		policy->min = get_input_boost_freq(policy, state);
	}
	else
