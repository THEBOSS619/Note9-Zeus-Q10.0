...
static ssize_t queue_rq_affinity_show(struct request_queue *q, char *page)
{
	...
	bool read = test_bit(QUEUE_FLAG_SAME_SYNC, &q->queue_flags);
	unsigned long val = 0;
...
queue_rq_affinity_store(struct request_queue *q, const char *page, size_t count)
{
	...
	if (val & SAME_SYNC_BIT)
		queue_flag_set(QUEUE_FLAG_SAME_SYNC, q);
	else
		queue_flag_clear(QUEUE_FLAG_SAME_SYNC, q);
