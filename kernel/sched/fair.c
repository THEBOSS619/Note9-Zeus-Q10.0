...
unsigned int sysctl_sched_capacity_margin_up[MAX_MARGIN_LEVELS] = {
	[0 ... MAX_MARGIN_LEVELS - 1] = 1880
}; /* ~60% margin */
unsigned int sysctl_sched_capacity_margin_down[MAX_MARGIN_LEVELS] = {
	2085, 1880
}; /* ~61 margin for big, ~49% margin for big+ */

#if NR_CPUS == 8
unsigned int sched_capacity_margin_up[NR_CPUS] = {
	1880, 1880, 1880, 1880, 1880, 1880, 1880, 1478
}; /* ~60% margin for small and big, 20% for big+ */
unsigned int sched_capacity_margin_down[NR_CPUS] = {
	1880, 1880, 1880, 1880, 2085, 2085, 2085, 1880
}; /* not used for small cores, ~61% margin for big, ~49% margin for big+ */
#else
unsigned int sched_capacity_margin_up[NR_CPUS] = {
	[0 ... NR_CPUS-1] = 1680}; /* ~20% margin */
unsigned int sched_capacity_margin_down[NR_CPUS] = {
	[0 ... NR_CPUS-1] = 1680}; /* ~20% margin */
#endif
 ...
 * If p is boosted, or is an ontime task, prefer that p runs on a faster cpu;
 * otherwise, allow p to run on any cpu.
 */
static inline bool
