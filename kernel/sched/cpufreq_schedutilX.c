...
static void sugov_irq_work(struct irq_work *irq_work)
{
	 ...
	 * the schedutilX governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor schedutilX_gov;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (policy->up_transition_delay_us && policy->down_transition_delay_us) {
		...
		unsigned int lat;
                ...
                tunables->down_rate_limit_us = 100;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDUTILX
struct cpufreq_governor *cpufreq_default_governor(void)
{
	return &schedutilX_gov;
}
#endif

static int __init sugov_register(void)
{
	return cpufreq_register_governor(&schedutilX_gov);
}
